...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        img_patches = self.layer4(x) # (B, 2048, 7 , 7)
        x = self.avgpool(img_patches) # (B, 2048, 1, 1)
        x = torch.flatten(x, 1) # (B, 2048)
        # x = self.fc(x)

        return x, img_patches
...
class BCLModel(nn.Module):
    ...
    def forward(self, x):
        feat, img_patches = self.encoder(x) # (B, 2048, 7, 7), (B, 2048)
        feat_mlp = F.normalize(self.head(feat), dim=1) # (B, 1024)
        logits = self.fc(feat)
        centers_logits = F.normalize(self.head_fc(self.fc.weight.T), dim=1)
        return feat_mlp, logits, centers_logits, img_patches